...
<table id="user-table" class="table table-bordered dataTable dtr-inline" aria-describedby="example1_info"
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>Beban</td>
            <td>{{  Money::IDR(collect($acc_outgoing)->where('acc_type', 1)->sum('total'), true)   }}</td>
        </tr>
        <tr >
            <td style="background-color: rgba(220, 220, 220, 0.477);">Jumlah Aktiva Lancar</td>
            <td style="background-color: rgba(220, 220, 220, 0.477);">{{ Money::IDR(collect($acc_income)->whereIn('acc_type', [1,2,4])->sum('total'), true) }}</td>
        ...
        <tr>
            ...
            <td style="background-color: rgba(220, 220, 220, 0.477);"> {{ Money::IDR(collect($acc_outgoing)->whereIn('acc_type', [1,2,3])->sum('total'), true) }} </td>
